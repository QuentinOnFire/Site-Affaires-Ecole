import "/module/return_semaine.js";
...
document.getElementById("submit_all").addEventListener("submit", function () {
    ...
    if (lv_value === "" || latin_value === "" || jour_value === "") {
        ...
        var lundi = fr + eps + p_i(maths, lv) + v_d_class + art + latin();
        var mardi = an + h_g_emc + fr + maths + lv + p_i(physique, techno);
        var mercredi = h_g_emc + svt + maths + fr;
        var jeudi = an + fr + lv + physique + pasto + techno + latin() + devoir_fait;
        var vendredi = p_i(fr, svt) + an + maths + h_g_emc + musique + latin();
        var head = `
